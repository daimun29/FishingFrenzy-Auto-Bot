...
// Input form untuk jumlah Susi
const susiInputForm = blessed.form({
  parent: menuContainer,
  top: 0,
  left: 0,
  width: "100%",
...
const susiInputLabel = blessed.text({
  parent: susiInputForm,
  top: 1,
  left: 1,
  content: "Enter quantity of Susi to use:",
});

const susiInput = blessed.textbox({
  parent: susiInputForm,
  top: 3,
  left: 1,
  width: "50%",
...
// Tombol Submit untuk Susi
const susiSubmitButton = blessed.button({
  parent: susiInputForm,
  top: 7,
  left: 1,
  width: 10,
...
// Tombol Back untuk Susi Input
const susiBackButton = blessed.button({
  parent: susiInputForm,
  top: 7,
  left: 12,
  width: 10,
...
async function fetchInventory(token, userId) {
  try {
    ...
    if (data.status === "success") {
      ...
      addLog(`Failed to fetch inventory: ${data.message}`);
      return null;
    }
  } catch (error) {
    addLog(`Error fetching inventory: ${error.message}`);
    return null;
  }
}
...
async function updateUserInfo() {
  ...
  const inventory = await fetchInventory(activeToken, userProfile.id);
  ...
  const boxCount = inventory && inventory.chests ? inventory.chests.length : 0;
  const susiCount = inventory && inventory.items?.find(item => item.id === "668d070357fb368ad9e91c8a")?.quantity || 0;
...
async function openAllChests(token, userId) {
  ...
  for (const chest of inventory.chests) {
    try {
      ...
      const data = await response.json();
      if (data.status === "success") {
        addLog(`Successfully opened chest ${chest.id}: ${JSON.stringify(data.data)}`);
      } else {
        addLog(`Failed to open chest ${chest.id}: ${data.message}`);
      }
    } catch (error) {
      addLog(`Error opening chest ${chest.id}: ${error.message}`);
    }
  }
...
// Use Susi
async function useSusi(token, userId, quantity) {
  try {
    ...
    const data = await response.json();
    if (data.status === "success") {
      addLog(`Successfully used ${quantity} Susi: ${JSON.stringify(data.data)}`);
    } else {
      addLog(`Failed to use Susi: ${data.message}`);
    }
  } catch (error) {
    addLog(`Error using Susi: ${error.message}`);
  }
  await updateUserInfo();
}
...
function showFishingPopup() {
  ...
  fishingList.on('select', (item, index) => {
    ...
    promptBox.setFront();
    screen.render();
    promptBox.readInput("Enter number of fishing attempts:", "", async (err, value) => {
      if (err || !value) {
...
inventoryMenu.on('select', async (item, index) => {
  const allTokens = fs.readFileSync("token.txt", "utf8")
    ...
    .filter(line => line !== "");
  if (allTokens.length === 0) {
    addLog("No tokens found in token.txt.");
    return;
  }

  if (index === 0) { // Box
    addLog("Opening all chests for all users...");
    for (const token of allTokens) {
      const userProfile = await fetchUserProfile(token);
      if (userProfile) {
        addLog(`Opening chests for user: ${userProfile.username}`);
        await openAllChests(token, userProfile.id);
      }
    }
  } else if (index === 1) { // Susi
    inventoryMenu.hidden = true;
    susiInputForm.hidden = false;
    susiInput.focus();
    screen.render();
...
susiSubmitButton.on('press', async () => {
  const quantity = parseInt(susiInput.getValue(), 10);
  if (isNaN(quantity) || quantity <= 0) {
    addLog("Please enter a valid quantity.");
    return;
  }
  ...
  addLog(`Using ${quantity} Susi for all users...`);
  for (const token of allTokens) {
    const userProfile = await fetchUserProfile(token);
    if (userProfile) {
      addLog(`Using ${quantity} Susi for user: ${userProfile.username}`);
      await useSusi(token, userProfile.id, quantity);
    }
  }

  susiInputForm.hidden = true;
  inventoryMenu.hidden = false;
  inventoryMenu.focus();
  screen.render();
});

susiBackButton.on('press', () => {
  susiInputForm.hidden = true;
  inventoryMenu.hidden = false;
  inventoryMenu.focus();
  screen.render();
